...
```{r - Investigating the data set variables (added Adam's stuff here)}
library(ggplot2)
library(dplyr)
...
## Songs Played Per Day, Subscription Type, and Churn tests and graphs
...
```
The minimum number of songs played per day are 1 and the maximum songs played per day are 99. There might be a relationship here! First, let's test whether songs played per day has a relationship with whether someone churns. We use an ANOVA test because we are comparing a quantitative variable (songs played per day) with a factor variable (churn).
```{r}
...
ggplot(world_data, aes(fill = churn_ratio)) +
  geom_sf() +
  labs(title = "Global Churn Ratio by Country")
```

```{r}
dense_ranks <- dense_rank(comparison$ratio)

actualrankings <- c("IN","UK","CA","US","AU","FR","DE","PK")
