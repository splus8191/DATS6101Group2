...
Mean skip rates are almost identical (~0.30) across all subscription types with similar variability and balanced sample sizes.
Visually and descriptively, no clear difference appears.
...
Although no significant relationships were found between individual categorical variables and churn, with p-values not reaching conventional levels of significance (all p > 0.2), exploring interactions between multiple categorical variables revealed some potential associations. Specifically, the interaction between country and gender when crossed with device type suggested relations worthy of further exploration. Statistical tests comparing means and variances across groups consistently showed similar results, indicating no significant differences in these measures. These findings suggest that while single factors alone may not significantly predict churn, combinations of variables could provide more insight, highlighting the importance of considering interactions in churn analysis. Additionally, skip rate showed no meaningful difference between churned and active users or across subscription types, further reinforcing the absence of strong behavioral patterns within this dataset.
