...
```{r}
airdata <- read.csv("sample airline data.csv")

colSums(is.na(airdata))
sum(is.na(airdata))
```

We first opened up the file and examined the data using the str() function. We checked to see if there are any missing values in the data, which there are 2,425,691 across different columns and rows. There are 539,747 observations in this data set, and 47 variables. This data is flight specific during the month of January 2025. The variables are:
...
```{r}
airlinecode <- c('DL', 'WN', 'B6', 'UA', 'AA', 'G4', 'F9', 'HA', 'AS', 'MQ', 'NK', 'OH', 'OO', 'YX')

airlinecode1 <- data.frame(
  OP_UNIQUE_CARRIER = c('DL', 'WN', 'B6', 'UA', 'AA', 'G4', 'F9', 'HA', 'AS', 'MQ', 'NK', 'OH', 'OO', 'YX'),
  airline = c('Delta', 'Southwest', 'JetBlue', 'United', 'American', 'Allegiant',
              'Frontier', 'Hawaiian', 'Alaska', 'EnvoyAir', 'Spirit', 'PSA', 'SkyWest', 'Republic')
)
...
## Chi-square Test: Airline vs Overall Delay Frequency
```{r}
